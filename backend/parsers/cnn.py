...
class CNNParser(WebParser):
  ...
  def __init__(self):
    ...
    self.disqus_info_url = 'http://disqus.com/api/3.0/threads/details?api_key=%(api_key)s&thread:ident=%(ident)s&forum=cnn'
    self.disqus_url = 'http://disqus.com/api/3.0/threads/listPostsThreaded?limit=100&thread=%(thread)s&forum=cnn&order=asc&cursor=%(cursor)s&api_key=%(api_key)s'
    self.baseurl = None
    self.collection = 'cnn.com'
    self.domain = 'news'
    self.url = None
    self.title = None
    self.author = None
  ...
  def _get_disqus_id(self):
    disqus_ident = urlparse.urlparse(self.url).path
    info_url = self.disqus_info_url % {'api_key':self.disqus_api_key, 'ident':disqus_ident}
  ...
  def process_post(self, post_url):
    ...
    print '---- Post ----'
    print self.url
    self.post_id = 'cnn%s' % self.disqus_id
    try:
      ...
      #author
      try:
        self.author = head.find('meta',attrs={'name':'author'})['content']
      except Exception, e:
        self.author = None
      #date
      try:
        raw_date = head.find('meta',attrs={'name':'pubdate'})['content']
      ...
      #title
      try:
        self.title = head.find('meta',attrs={'itemprop':'headline'})['content']
      except Exception, e:
        self.title = None
      soup = BeautifulSoup(soup.prettify(), "lxml", parse_only=SoupStrainer('div','cnn_strycntntlft'))
  ...
  def process_comments(self, csoup):
    for comment in csoup:
      ...
      i.post_author_id = 'cnn%s' % comment['author']['id'] if not comment['author']['isAnonymous'] else 'cnn#%s' % comment['author']['name']
