...
class OnetBiznesParser(WebParser):
  def __init__(self):
    self.nested = False
    self.post_id = None
    self.baseurl = None
    self.post_path = None
    self.print_url = 'http://biznes.onet.pl/,0,%(post_id)s,drukuj'
  ...
  def process_post(self, post_url):
    self.items.clear()
    self.url = post_url
    post_id = re.findall('\d\d+',post_url)[-1]
    if post_id:
    ...
    try:
      ...
      resp = requests.get(self.print_url % {'post_id': self.post_id})
      printpage = resp.text
      resp.close()
    except Exception, e:
      ...
      head = soup.head
      body = BeautifulSoup(soup.prettify(), 'lxml', parse_only=SoupStrainer('div', id='cLeft'))
      ...
      content = BeautifulSoup(printpage, 'lxml', parse_only=SoupStrainer('div', id='bDetailText'))
      ...
      try:
        ...
        self.comments_url = 'http://%s%s' % (self.baseurl, forum_tag.find('div','k_nForumReaderMenu').find('a',text=re.compile('w.tki'))['href'])
